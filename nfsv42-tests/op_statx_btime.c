 ...
 *
 *   2. Birth time plausible.  btime.tv_sec is within +/- 60
 ...
 * The NFS client sets st_birthtimespec only when the server returns the
 * NFSv4.2 time_create attribute; a zero value means not available.
 */
...
next:
	...
	/* Case 1: server returns a non-zero birth time. */
	struct stat st;
	if (lstat(name, &st) != 0) {
	...
	if (lstat(name, &st3) == 0) {
		if ((st3.st_birthtimespec.tv_sec != 0
		     || st3.st_birthtimespec.tv_nsec != 0)
		    && (st3.st_birthtimespec.tv_sec != st.st_birthtimespec.tv_sec
